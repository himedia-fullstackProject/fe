...
const Health = () => {
  ...
  // 현재 페이지에 표시할 포스트
  const currentPostsBySubCategory = Object.keys(postsBySubCategory).reduce((acc, subCategoryId) => {
    acc[subCategoryId] = postsBySubCategory[subCategoryId].slice(indexOfFirstPost, indexOfLastPost);
    return acc;
  }, {});

  // 전체 포스트 수 계산
  const totalPosts = Object.values(postsBySubCategory).reduce((total, posts) => total + posts.length, 0);
  // 총 페이지 수 계산
      ...
      {subCategories.map(subCategory => (
        <div key={subCategory.id}>
          ...
          <div className={styles.grid}>
            ...
            {currentPostsBySubCategory[subCategory.id]?.length === 0 && <p>포스트가 없습니다.</p>}
          </div>
        </div>
      ))}
